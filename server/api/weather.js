...
class Weather {
    ...
    /**
     * Gets the weather data baesd on the zipcode and which temp system to convert to (imperial/metric)
     * 
     * @param {number} zipCode The zipcode used to get the weather info from the weather api
     * @param {string} tempImperial This is either "imperial" (use Fahrenheit) or "metric" (use Celsius)
     * @return {JSON} The data response from the weather api call.
     */
    getWeatherData = async(zipCode, tempImperial) => {
         ...
         */
        let url = `${baseUrl}?zip=${zipCode},us&appid=${process.env.WEATHER_KEY}&units=${tempImperial}`;

        // Awatable call to get the information from the weather api and then return the data.
